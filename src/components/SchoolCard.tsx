      ...
      <div className="flex justify-between items-start mb-3">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`badge ${typeColors[school.type] || 'bg-gray-100 text-gray-700'}`}>{typeLabels[school.type] || school.type}</span>
            {school.gender && (
          ...
          <h3 className="text-lg font-bold text-gray-900 group-hover:text-primary-700 transition">{school.name_zh || school.name_en}</h3>
          <p className="text-sm text-gray-500 line-clamp-2">{school.name_en}</p>
        </div>
      </div>
      <div className="space-y-1.5 text-sm text-gray-600">
        {school.district && <div className="flex items-center gap-2"><span>📍</span><span>{school.district}</span></div>}
        {school.medium_of_instruction && <div className="flex items-center gap-2"><span>🎓</span><span>{school.medium_of_instruction}</span></div>}
        ...
        {school.application_deadline && <div className="flex items-center gap-2"><span>📅</span><span className="text-red-600 font-medium">截止: {school.application_deadline}</span></div>}
      </div>
      {school.notable_features && school.notable_features.length > 0 && (
